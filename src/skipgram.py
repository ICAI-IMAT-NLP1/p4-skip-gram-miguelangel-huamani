...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        # TODO
        noise_words: torch.Tensor = torch.multinomial(noise_dist, batch_size*n_samples, replacement=True)  # numero de muestras negativas = num_samples * cada palabra del batch 

        device: str = "cuda" if self.out_embed.weight.is_cuda else "cpu"
        #device: str = "mps" if torch.backends.mps.is_available() else "cpu"  # Aprovecho la GPU del Mac
        noise_words: torch.Tensor = noise_words.to(device)

        noise_words = noise_words.view(batch_size, n_samples)       # para cada palabra del batch por fila, sus palabras negativas por columnas
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        loss = -(out_loss + noise_loss) / input_vectors.size(0)
        return loss